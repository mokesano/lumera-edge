...
#  <stable_branch>: the stable branch to release from (e.g. app-stable-2_4_5)
...
# Determine the tag of the last release on this branch (e.g. app-2_4_5-0)
...
# Calculate the tag of the next release (e.g. app-2_4_5-1
...
sed -i	-e "s/<tag>.*<\/tag>/<tag>${THISTAG}<\/tag>/" \
	...
	-e "s/<\/version>/	<patch from=\"${MAJOR}.${MINOR}.${REVISION}.${LASTBUILD}\">http:\/\/pkp.sfu.ca\/${APPLICATION}\/download\/patch\/app-${LASTPATCHSUFFIX}_to_${MAJOR}.${MINOR}.${REVISION}-${THISBUILD}.patch.gz<\/patch>\n<\/version>/" \
