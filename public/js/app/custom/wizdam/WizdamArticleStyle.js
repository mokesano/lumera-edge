...
$(document).ready(function() {
    ...
    const extractTitle = content => {
        ...
        if (yearMatch) {
            ...
            return content.substring(titleStart, titleEnd).trim()
                .replace(/&/g, '&amp;')
                .replace(/</g, '&lt;')
                .replace(/>/g, '&gt;')
                .replace(/"/g, '&quot;')
    ...
    const processVisibleReferences = () => {
        $('.reference').each(function() {
            ...
            const isDoiUrl = urlCandidate => {
                try {
                    const normalizedUrl = /^www\./i.test(urlCandidate) ? 'https://' + urlCandidate : urlCandidate;
            ...
            if (httpMatches) {
                httpMatches.forEach(httpUrl => {
                    if (!isDoiUrl(httpUrl)) addLinkWithLoading(referenceLinks, httpUrl, 'View Source', '<svg focusable="false" viewBox="0 0 8 8" height="20" aria-label="Opens in new window" class="icon icon-arrow-up-right-tiny arrow-external-link"><path d="M1.12949 2.1072V1H7V6.85795H5.89111V2.90281L0.784057 8L0 7.21635L5.11902 2.1072H1.12949Z"></path></svg>');
                });
            }
